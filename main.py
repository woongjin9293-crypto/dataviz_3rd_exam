...
NEWS_PATH = "/Users/jinwoong/kpop_demon_hunters_news.csv"
BLOG_PATH = "/Users/jinwoong/kpop_demon_hunters_blog.csv"
...
def load_data(path: str, source: str) -> pd.DataFrame:
    ...
    # 날짜가 깨진 행이 있으면 이후 필터에서 문제가 생길 수 있어서 먼저 정리한다
...
# 뉴스와 블로그는 글 스타일이 달라서 둘 다 준비해두고 선택해서 볼 수 있게 만든다
...
# 뉴스만 볼지, 블로그만 볼지, 둘 다 합쳐서 볼지 선택할 수 있게 한다
...
# 데이터가 실제로 존재하는 날짜 안에서만 고르게 해야 화면이 깨지지 않는다
...
# 처음에는 전체 범위를 한 번에 볼 수 있게 기본값을 잡는다
...
def clean_text_ko(text: str) -> str:
    # 쓸데없는 기호나 태그는 단어 뽑을 때 방해가 되어서 먼저 제거한다
    text = re.sub(r"<[^>]+>", "", text)
...
def extract_nouns(text: str, stopwords: set) -> list[str]:
    # 핵심 단어만 뽑아야 주제 흐름이 보이기 때문에 명사 위주로 가져온다
...
# 검색어 자체는 거의 항상 나오기 때문에 핵심 요인을 가리는 경우가 많아서 제외한다
base_stop = set(["데몬", "헌터스", "넷플릭스", "k팝", "케이팝", "관련", "이번", "통해", "기자"])
stopwords = set(base_stop)

# 문서마다 단어 목록을 만들어두면 뒤에서 빈도, 감성, 네트워크를 같이 돌릴 수 있다
all_nouns = []
...
if len(df) == 0:
    ...
    # 날짜별로 묶으면 관심이 언제 집중됐는지 한눈에 볼 수 있다
    trend = df.copy()
...
def sentiment_score_keyword(nouns: list[str]) -> int:
    # 주제 단어 중심으로 점수를 만들면 무엇 때문에 반응이 나왔는지 보기 쉽다
    score = 0
    for w in nouns:
        if w in theme_pos:
            score += 1
...
if len(df) == 0:
    ...
    # 같은 데이터라도 점수 기준이 달라지면 결과가 달라질 수 있어서 선택 값을 반영한다
...
# 전체 단어를 한 리스트로 모아야 많이 나온 단어를 쉽게 뽑을 수 있다
...
if len(flat) == 0:
    ...
    # 많이 나온 단어부터 보여주면 사람들이 뭘 중심으로 얘기하는지 빠르게 잡힌다
...
if len(flat) == 0:
    ...
    # 한글이 깨지지 않게 폰트를 먼저 잡아둔다
    try:
        han_font_path = font_manager.findfont("AppleGothic")
    except:
        han_font_path = None

    # 단어를 한 문장처럼 합쳐야 워드클라우드가 만들어진다
    wc = WordCloud(
...
if len(flat) == 0:
    ...
    # 핵심 키워드만 남기면 관계망이 너무 복잡해지지 않는다
    top_keywords = set([w for w, _ in freq.most_common(top_n_nodes)])

    # 같은 글에서 같이 나온 단어는 같은 흐름에서 언급됐을 가능성이 높다
    edge_list = []
    ...
    # 너무 드문 연결은 우연일 수 있어서 일정 기준 이상만 남긴다
    ...
    if G.number_of_nodes() == 0:
        ...
        # 연결된 단어끼리는 가까이 보이게 배치하면 흐름이 눈에 들어온다
